...
export default function WorldMap({selected,compare,tripStart,tripEnd,viewMode,showStreetNames,showCities,showZones,brightness,onSelect,onContextLocation,onZoom}:{selected:City;compare:City|null;tripStart:City|null;tripEnd:City|null;viewMode:'satellite'|'map'|'street';showStreetNames:boolean;showCities:boolean;showZones:boolean;brightness:number;onSelect:(city:City)=>void;onContextLocation:(lat:number,lng:number,x:number,y:number)=>void;onZoom:(value:number,lat:number,lng:number)=>void}){
 ...
 const selectRef=useRef(onSelect);const contextRef=useRef(onContextLocation);const zoomRef=useRef(onZoom);const lastNearby=useRef('');const previousSelection=useRef(`${selected.lat},${selected.lng}`);const [ready,setReady]=useState(false);const [nearby,setNearby]=useState<City[]>([]);const [streetZoom,setStreetZoom]=useState(1.7)
 ...
 useEffect(()=>{
  ...
  for(const layer of streetLabelLayers){
   ...
   const minimum=satellite?(layer.id==='highway-name-major'?10:layer.id==='highway-name-minor'?12:layer.id==='highway-name-path'?13:layer.minzoom??0):layer.minzoom??0
 ...
 useEffect(()=>{
  const map=mapRef.current;if(!map||!ready)return
  const update=()=>setStreetZoom(map.getZoom())
  map.on('zoom',update)
